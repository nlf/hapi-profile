...
internals.formatTime = function (time) {

    return (1000 * time[0]) + (time[1] / 1000000);
};

internals.diffTime = function (after, before) {

    return internals.formatTime([after[0] - before[0], after[1] - before[1]]);
};
...
internals.report = function (server, request) {
    ...
    return function (result) {
        ...
        for (var i = 0, il = result.ticks.length; i < il; ++i) {
            ...
            var stackLine = Util.format('%d: %dms %s (%d)', internals.diffTime(tick.start, result.start), internals.formatTime(tickTimes), internals.getLineFromStack(tick.stack), internals.formatTime(tick.overhead));
        ...
        server.log(['hapi-profile'], { request: request.id, stack: stack.join('\n') });
    };
};
